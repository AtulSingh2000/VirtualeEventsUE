...
bool FNDIConnectionInformation::operator==(const FNDIConnectionInformation& other) const
{
	...
	return this->Bandwidth == other.Bandwidth && this->bIsCurrentlyConnected == other.bIsCurrentlyConnected &&
...
FNDIConnectionInformation::operator NDIlib_recv_create_v3_t() const
{
	...
	NDIlib_source_t ndi_connection_source =
		NDIlib_source_t(TCHAR_TO_UTF8(*this->SourceName), TCHAR_TO_UTF8(*this->Url));
	...
	NDIlib_recv_bandwidth_e recv_bandwidth = this->Bandwidth == ENDISourceBandwidth::MetadataOnly
												 ? NDIlib_recv_bandwidth_metadata_only
												 : this->Bandwidth == ENDISourceBandwidth::AudioOnly
													   ? NDIlib_recv_bandwidth_audio_only
													   : this->Bandwidth == ENDISourceBandwidth::Lowest
...
FArchive& FNDIConnectionInformation::Serialize(FArchive& Ar)
{
	...
	return Ar << current_version << this->Bandwidth << this->bIsCurrentlyConnected << this->MachineName
			  << this->SourceName << this->StreamName << this->Url;
}
