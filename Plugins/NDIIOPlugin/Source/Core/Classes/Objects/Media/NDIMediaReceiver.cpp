...
void UNDIMediaReceiver::UpdateTimecode()
{
	...
	this->SyncTimecode = FTimecode::FromTimespan(FTimespan::FromSeconds(SystemTime / (float)1e+7), FrameRate,
...
void UNDIMediaReceiver::DrawVideoFrame(FRHICommandListImmediate& RHICmdList, NDIlib_video_frame_v2_t& Result)
{
	...
	if (p_framesync_instance != nullptr)
	{
		...
		if (!RenderTargetDescriptor.IsValid() ||
			RenderTargetDescriptor.GetSize() != FIntVector(Result.xres, Result.yres, 0))
		{
			...
			RenderTargetDescriptor = FPooledRenderTargetDesc::Create2DDesc(
				FrameSize, PixelFormat, FClearValueBinding::None, TexCreate_None, TexCreate_RenderTargetable, false);

			// Create a temporary targetable texture
			FTexture2DRHIRef InputTarget =
				RHICreateTexture2D(FrameSize.X, FrameSize.Y, PixelFormat, 1, 1, TexCreate_CPUReadback, CreateInfo);

			// Update the shader resource for the 'SourceTexture'
			RHICreateTargetableShaderResource2D(FrameSize.X, FrameSize.Y, PixelFormat, 1, TexCreate_Dynamic,
												TexCreate_RenderTargetable, false, CreateInfo, InputTarget,
												SourceTexture);
