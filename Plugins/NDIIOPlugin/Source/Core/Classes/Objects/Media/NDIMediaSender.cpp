...
UNDIMediaSender::UNDIMediaSender(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {}
...
void UNDIMediaSender::ChangeRenderTargetConfiguration(FIntPoint InFrameSize, FFrameRate InFrameRate)
{
	...
	ReadbackTexture =
		RHICreateTexture2D(FrameSize.X, FrameSize.Y, PF_B8G8R8A8, 1, 1, TexCreate_CPUReadback, CreateInfo);

	// Create the RenderTarget descriptor
	RenderTargetDescriptor = FPooledRenderTargetDesc::Create2DDesc(FrameSize, PF_B8G8R8A8, FClearValueBinding::None,
